...
|---|---|---|
| `fail-on` | *(lockrot's own default: `none`)* | Verdict or priority that fails the step: `none`, `stale`, `old-promise`, `pinned`, `silent`, `abandoned`, or `low`, `medium`, `high`, `critical` (a priority reads the package's place in the project, so `high` fails on an abandoned direct requirement and passes the same verdict in a transitive dev package). Empty defers to `extra.lockrot.fail-on` in `composer.json` |
